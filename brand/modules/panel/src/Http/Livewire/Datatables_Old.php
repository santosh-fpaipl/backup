...
class Datatables_Old extends Component
{
    // Pagination
    use WithPagination;
    public $pageLength;
    ...
    public $sortInOrder;
    public $sortSelect;
    public $sort_select_default = 'created_at#asc';
    ...
    public function fetchData($flag = '')
    {
        ...
        // Default Sorted
        if (in_array($this->sortByColumn, $modelObj->getFillables())) {
            $query = $query->orderby($this->sortByColumn, $this->sortInOrder);
        }
        ...
        if ($flag == 'export') {
            ...
            $datas = $query->get();
        } elseif (!in_array($this->sortByColumn, $modelObj->getFillables())) {
            $datas = $query->get();
        ...
        if (in_array($this->sortByColumn, $modelObj->getFillables())) {
            ...
            if ($this->sortInOrder == 'asc') {
                $datas->sortBy($this->sortByColumn);
            } else {
                $datas->sortBy($this->sortByColumn)->reverse();
            }
            // Manuaaly create pagination links
            
                $itemsPerPage = 5;
                $currentPage = 1; // The current page number

                // Calculate the total number of items
                $totalItems = count($datas->toArray());
                // if ($modelObj->hasExtraFields()) {
                //     dd($modelObj->getExtraFields());
                //     $newDatas = array_merge($datas->toArray(), $modelObj->getExtraFields());
                // } else {
                //     $newDatas = $datas->toArray();
                // }

                // dd($newDatas);
                // Slice the data based on the current page and items per page
                $slicedDatas = array_slice($datas->toArray(), ($currentPage - 1) * $itemsPerPage, $itemsPerPage);

                $slicedData = collect();
                foreach ($slicedDatas as $arrayData) {
                    if ($modelObj->hasExtraFields()) {
                        $arrayData = $modelObj->getExtraFields($arrayData);
                    }
                    $slicedData->push((object) $arrayData);
                }

                // Create the paginator instance
                $paginator = new LengthAwarePaginator($slicedData, $totalItems, $itemsPerPage, $currentPage);

                $customPath = 'http://127.0.0.1:8000/suppliers'; // Replace this with your desired custom path
                $options = ['path' => $customPath, 'query' => request()->query()];
                $paginator->setPath($customPath);
                $paginator->withPath(url()->current())->appends($options);

                //dd($paginator);
                // You can now use the $paginator instance to render the pagination links in your view.
                return $paginator;
